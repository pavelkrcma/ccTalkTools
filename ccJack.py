    ...
    try:
        ...
        if len(times)>0:
            ...
            print("[*] Replacing device @ "+str(options.source)+\
                    " as device @ "+str(options.destination))
            ...
            while True:
                ...
                userHeader = input("Value : ")
                if userHeader == 'l':
                    for header, response in responder.getResponses():
                        print(header, " : ", binascii.hexlify(response.encode() if isinstance(response, str) else response).decode())
                elif userHeader.isdigit():
                    userData = binascii.unhexlify(input('Enter new data (in hex): '))
